...
def dryrun(all_param_dic: dict) -> dict:
    ...
    for yml_item in YML_ORDER:
        ...
        result[str(yml_item)] = process_yml(yml_item, all_param_dic, deploys=False)
...
def deploy(all_param_dic: dict) -> dict:
    ...
    for yml_item in YML_ORDER:
        print(f"***** {yml_item} start *****")
        result[str(yml_item)] = process_yml(yml_item, all_param_dic, deploys=True)
        print(f"***** {yml_item} finished *****")
    return result
...
def process_yml(yml_item: YmlItem, all_param_dic: dict, deploys: bool) -> List:
    ...
    if ENV == "prd":
        ...
        stack_name = f"{ENV}-{PROJECT_NAME}-{stack_suffix}3"
    ...
    try:
        waiter.wait(
            ChangeSetName=change_set_name,
            StackName=stack_name,
            WaiterConfig={"Delay": 3, "MaxAttempts": 50},
        )
        ...
        waiter.wait(StackName=stack_name, WaiterConfig={"Delay": 3, "MaxAttempts": 50})

        return [x["ResourceChange"] for x in desc_response["Changes"]]

    except botocore.exceptions.WaiterError as err:
        # - 何も差分がないときのerr(Failedになってしまう仕様です...実際には差分ないのでエラーではないです。)
        ...
        print("WaiterError err(empty-changeset causes FAILED): ", err)
        if "UPDATE_ROLLBACK_COMPLETE" in str(err):
            print("UPDATE_ROLLBACK_COMPLETE")
...
def upload_yml_to_s3(yml_filename: str, region: str) -> str:
    ...
    s3_client.upload_file(yml_filename, s3_destination_bucket, yml_filename)
    s3_source_signed_url = s3_client.generate_presigned_url(
        "get_object",
        Params={"Bucket": s3_destination_bucket, "Key": yml_filename},
...
def create_stack(client, stack_name: str, template_url: str, parameters: List) -> None:
    ...
    waiter.wait(StackName=stack_name, WaiterConfig={"Delay": 3, "MaxAttempts": 120})
...
def delete_pull_request_comment(comment_url: str) -> None:
    ...
    response = requests.delete(
        comment_url, headers={"Authorization": f"token {GITHUB_TOKEN}"}
    )
...
def post_to_pull_request(body: str) -> None:
    ...
    response = requests.post(
        URL, json={"body": body}, headers={"Authorization": f"token {GITHUB_TOKEN}"}
    )
